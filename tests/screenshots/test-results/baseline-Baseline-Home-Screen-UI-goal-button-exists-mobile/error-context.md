...
- Name: baseline.spec.ts >> Baseline: Home Screen UI >> goal button exists
- Location: tests/e2e/specs/baseline.spec.ts:36:7
...
Locator:  locator('#goal-modal-btn')
Expected: visible
Received: hidden
Timeout:  5000ms
...
Call log:
  ...
  - waiting for locator('#goal-modal-btn')
    9 × locator resolved to <button title="ゴール＆タスク" id="goal-modal-btn" onclick="openGoalModal()">G</button>
