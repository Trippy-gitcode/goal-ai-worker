...
- **UI変更フロー（A9）:** 新規=Claude.aiがビジュアライザーでデザイン案→ふとし承認→Codeがmockup HTML化+実装。変更=Codeが差分調査レポート→Claude.aiが推奨判断付きで提示→ふとし判断→Codeがmockup更新+実装修正+C16検証。Claude.aiはHTML編集しない（鉄則2・7）。ファイル書き込みはふとし承諾後（鉄則追加）
