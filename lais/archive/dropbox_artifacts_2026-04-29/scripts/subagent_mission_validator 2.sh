...
# --- ミッションタイプ判定（PATCH-VALIDATOR-OVERFIRE-REDUCTION-V1、2026-04-27） ----
# 目的セクション内のキーワードでタイプを判定し、impl 以外なら MATRIX 検証を skip。
# skip 時は代替検証（ファイル新設 / 行数 / セクション grep / 分析レポート出力等）を要求。
#
# タイプ:
...
#
# 判定優先度: impl 優位キーワード > spec キーワード > analysis キーワード > 既定 impl
...
MISSION_TYPE="impl"
# impl 優位（明示的実装系キーワードがあれば impl 確定）
if printf '%s' "$PURPOSE_BLOCK" \
   | grep -qE "(実装|デプロイ|機械強制 ?hook 改修|hook 改修|動作テスト 6|hook 再結線|改修箇所)"; then
  MISSION_TYPE="impl"
...
log "[INFO] mission_type=${MISSION_TYPE} purpose_block_len=$(printf '%s' "$PURPOSE_BLOCK" | wc -c | tr -d ' ')"
