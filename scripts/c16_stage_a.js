...
// Output: docs/mockups/screenshots/{folder}/impl_compare/sa_{compare,impl,mockup}.png
const { chromium } = require('playwright');
const path = require('path');
const fs = require('fs');

const SCREENS = [
  { id: 'a2_vision',        folder: 'a2_vision',     mockup: '06b_design_vision.html',   fragment: true,  implPage: 'myself', implTab: 'vision' },
  { id: 'a3_goal_link',     folder: 'a3_goal',       mockup: '06c_design_connect.html',  fragment: false, implPage: 'myself', implTab: 'connect' },
  { id: 'a4_profile',       folder: 'a4_prof',       mockup: '06d_design_profile.html',  fragment: false, implPage: 'myself', implTab: 'profile' },
  { id: 'a5_plan',          folder: 'a5_plan',       mockup: '08d_plan.html',            fragment: true,  implPage: 'plan' },
  { id: 'b1_hub_tasks',     folder: 'b1_tasks',      mockup: '05b_goalhub_tasks.html',   fragment: false, implPage: 'goalhub', implTab: 'tasks' },
  { id: 'b2_hub_settings',  folder: 'b2_settings',   mockup: '05e_goalhub_settings.html',fragment: false, implPage: 'goalhub', implTab: 'settings' },
  { id: 'b3_analytics',     folder: 'b3_analytics',  mockup: '08a_analytics.html',       fragment: true,  implPage: 'analytics' },
  { id: 'b4b5_fb_settings', folder: 'b4b5_fb_set',   mockup: '08b_feedback.html',        fragment: true,  implPage: 'settings' },
];
...
(async () => {
  ...
  for (const screen of SCREENS) {
    const outDir = path.join(BASE_DIR, screen.folder, 'impl_compare');
    ...
    try {
      ...
      if (phoneFrame) {
        await phoneFrame.screenshot({ path: path.join(outDir, 'sa_mockup.png') });
      } else {
        await mockupPage.screenshot({ path: path.join(outDir, 'sa_mockup.png'), fullPage: false });
      }
    ...
    try {
      ...
      await implPage.screenshot({ path: path.join(outDir, 'sa_impl.png'), fullPage: false });
    ...
    try {
      ...
      const mockImg = await loadImage(path.join(outDir, 'sa_mockup.png'));
      const implImg = await loadImage(path.join(outDir, 'sa_impl.png'));
      const gap = 20;
      ...
      fs.writeFileSync(path.join(outDir, 'sa_compare.png'), canvas.toBuffer('image/png'));
