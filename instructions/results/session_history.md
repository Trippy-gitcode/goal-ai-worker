...
- 2-4 スキップ（CRN-02確認: フッターモデル名grayが正。名前ヘッダー未実装のため対象なし）
- 2-6 ✅ GoalHub: ヘッダー8px 14px/タイトル13px/戻るテキストリンク/バー4px/タブ9px+gradient underline
- 2-8 ✅ プロフィールアバター 80→48px
- 1-3, 1-4 未着手（タスク画面大規模リファクタ — 要慎重実装）
- 2-2 ✅ ホーム会話前: ツールバー36→20px, プリセット12→10px/0.5px, 入力padding 4px→8px
- 2-3 ✅ ホーム会話中: アバター24→18px, フッター9→7px
- 2-5 ✅ タスク一覧: タイトル12.5→9px, チェック16→14px, カテゴリ13→11px
- **STEP 2 全完了**
- 3-4 ✅ AIロール「変更する」テキスト入力展開（GoalHub設定ペイン）
- 3-5 ✅ ビジョン再分析6チップUI（キャラクター/ビジョン/見られたい姿/強み/5年後/やりたくない）
- 3-6 ✅ Connect 60%以下オレンジ + 「改善方法をAIに相談する」ボタン
- 3-7 ✅ SNS共有「キャラクターをシェア」ボタン（Web Share API / clipboard fallback）
- 3-1 ✅ GoalHub分析 停滞ポイント（日付+内容+タグ色分け）
...
### テンプレート（候補記載時）
| # | タスク名 | 影響範囲 | 工数目安 | 優先度案 | UI変更 | 関連参照ドキュメント |
|---|---------|---------|---------|---------|:---:|-------------------|
| - | 例: ○○ | frontend/js/chat.js | 30分 | 🟡 | あり/なし | docs/goal_ai_project_v6_4.md §X |

※ **UI変更=「あり」の場合、Codeは承認されても自律実装しない。** Claude.aiチャットでmockup作成→ふとしビジュアルチェックOK→キューに移動（C18）
...
### 第5回調査（2026-03-24 キュー空時自律提案）

**spec準拠修正（テスト配布品質向上）**

| # | タスク名 | 影響範囲 | 工数目安 | 優先度案 | 関連参照ドキュメント |
|---|---------|---------|---------|---------|-------------------|
| BA | ゴール検出トースト位置修正（bottom→top, translateY(-100%)→0） | frontend/style.css, chat.js | 30min | 🟡 | design_amendment_001 #3 |
| BB | タスク完了C案アニメーション（shrink→flipY→紙吹雪5粒） | frontend/js/chat.js, style.css | 1.5h | 🟡 | design_spec_v3 P-33/T04A-07 |
| BC | ストリーミング停止ボタン — GoalHub/FB/タスクチャット画面 | frontend/js/goals.js, chat.js | 1h | 🟡 | design_amendment_001 #2 |
| BD | プラン選択UI 2ステップ化（選択→確認） | frontend/js/ui.js | 1h | 🟢 | project_v6_4 §8 |
| BE | テーマプレビューサムネイル | frontend/js/ui.js | 1h | 🟢 | project_v6_4 §8 |
| BF | プロフィール理解度スクロール自動閉じ | frontend/js/profile.js | 30min | 🟢 | reference_v2 #15 |

**注:** 第4回のAG〜AV候補のうち、以下は実装済みを確認:
- AJ(AIメモ空状態テキスト)、AN(メッセージ編集)、AO(アコーディオン)、AP(5年後/やりたくない)、AQ(再分析6チップ)、AR(Connect相談ボタン)、AS(キャラクターシェア)、AV(AIロール変更入力)、AW(AIメモ更新トースト)
...
2. **手順記憶依存の体系的排除（次回棚卸し時）** — 全Cルールを棚卸しし、「自動検証（canopy/pre-commit/スクリプト）でゲートされているか」を確認。ゲートされていないルールは自動検証追加 or C2基準フローに組み込み or スクリプト化のいずれかで対処。
