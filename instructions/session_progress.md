...
## 5行サマリー
- **Version:** v4.0.3（デプロイ済み 2026-04-03）
- **Next:** ユーザー操作テスト全件実施（docs/ux_user_test_v1.md）→ DEV-01 → DEV-02
- **Last done:** TEST-01 FAIL修正 v4.0.3。ドラッグ順序復元 + モデル名テキスト削除 + 正午リセット + ゴール作成TALK連携 + 残絵文字除去
- **Open issues:** ユーザー操作テスト未実施、BUG-01b残課題
- **方針変更:** 新コンセプト「毎朝、最善の一日を始めよう」承認。UX全面刷新（docs/ux_redesign_v1.md）+ 開発体制改善4Phase（docs/dev_improvement_v1.md）

## 現在地
- **バージョン:** v4.0.2（デプロイ済み）
- **チェーン:** TEST-01 → TEST-02（ルーティング500パターン）→ DEV-01 → DEV-02 → テスト配布準備
- **次のミッション:** ユーザー操作テスト全件実施（docs/ux_user_test_v1.md）

## ミッションキュー（上から順に実行）

### V4: ✅完了（v4.0.0デプロイ済み 2026-04-03）

---

### TEST-01: ユーザー操作テスト全件実施（次のミッション）
> リスク: 🟡中（テスト結果次第で追加修正発生）
> 参照: docs/ux_user_test_v1.md

全項目を順番にブラウザ/シミュレーターで実行。FAILは即修正→再テスト。全PASSまで。
スクショで動作確認（A7ルール）。grepだけで✅にしない。

---

### TEST-02: ルーティング500パターン検証
> リスク: 🟡中（結果次第でルーティングロジック改修）
> 参照: docs/routing_500_test.md

500件の質問を自動テストスクリプトで実行。合格基準: 全体90%以上、カテゴリ別85%以上、境界ケース80%以上。
未達の場合はquickRouteの正規表現追加＋callRoutingAPIのシステムプロンプト改善を実施→再テスト。

---

### DEV-01: デザインスキル導入（ふとし承認済み 2026-04-01）
> リスク: 🟡中（UI修正＋バージョンアップ）
> 参照: docs/ux_checklist_v1.md

**Step 1: チェックリスト残FAIL項目を修正**
- H-04: メモSupabase保存 — [MEMO_SAVE]フロント検出のフル実装
- H-05: やりたいことリスト — localStorage→Supabase移行（テスト配布後でもOK）
- H-07: sort_order — localStorage→Supabase移行（テスト配布後でもOK）
- B-18: カレンダーから日記閲覧 — カレンダー画面に日記表示エリア追加
- G-01〜03: 旧ページDOMが再利用中 — 不要な旧ID/クラスのクリーンアップ
- I-06: Caveat手書きフォント — スキップ承認済み（対応不要）

**Step 2: BUG-02（セクションJ）**
- J-01: TALK入力ボックス＋下タブ重なりバグ
- J-02: 入力ボックス＋チャットUX全体（吹き出し・メッセージ表示・タイムスタンプ・アイコン）をTALK画面に完全統一。TODAYとTALKでチャットの見た目が異なってはならない
- J-03〜05: 絵文字完全排除→SVG置換

**Step 2.5: BUG-03 ルーティング不具合（ふとし報告 2026-04-02）**
- 症状: 「東16のダイヤを教えて」（検索・事実系）がClaude Opusにルーティングされた。Geminiに行くべき
- 修正: ルーティングロジックで検索・事実・時刻表・路線・地理情報キーワードがGeminiルートに含まれていること確認。テストケースに「○○のダイヤ」「○○の時刻表」「○○への行き方」追加

**Step 2.6: タスク追加UX修正（ふとし指摘 2026-04-02）**
- +ボタンでTALKに遷移する現在の動作を変更
- ハーフモーダル（下からシート）でTODAY画面の上にAI会話を展開→タスク確定→シート閉じてTODAYに即反映
- [TASK_UPDATE]タグがチャットに生テキスト表示されるバグも修正（Worker側でパース→ストリップ）

**Step 2.7: 日記仕様修正（ふとし指摘 2026-04-02）**
...
**Step 2.8: ユーザー操作テスト全件実施**
- docs/ux_user_test_v1.md の全項目を実行（ブラウザ/シミュレーター）
- 最優先: [TASK_UPDATE]タグパースバグ（生テキスト表示＋タスク未作成）
- FAILは即修正→再テスト。全PASSまで繰り返し

**Step 3: バージョンアップ**
- APP_VERSION を v4.0.0 に更新（4箇所同期: C10ルール）
- git tag v4.0.0
- git push --tags
- デプロイ
