...
export function quickRoute(message) {
  ...
  if (/^(今日やること|タスク|やること.*整理|TODO|to.?do|スケジュール|予定.*整理|段取り)/.test(msg)) {
    return { route: 'gpt', coaching: false };
  }
  return null;
}
...
export async function callRoutingAPI(env, auth, userMessage) {
  try {
    const routeSystem = 'ユーザーのメッセージを分類せよ。以下のカテゴリから1単語のみ返せ: claude（深い感情サポート・人生相談・コーチング核心部・悩み・自己分析・価値観・モチベーション）, gemini（検索・天気・ニュース・事実Q&A・比較分析・データ処理・調査）, gpt-simple（相槌・短い返事・挨拶）, gpt（アイディア出し・一般会話・翻訳・要約・クリエイティブ・タスク相談・タスク整理・やること整理・スケジュール管理・その他すべて）';
